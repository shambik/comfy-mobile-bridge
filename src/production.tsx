...
function isNonMeaningfulTrace(message: Message) {
  if (message.kind !== 'agent_trace' || message.metadata?.stream === 'response' || message.metadata?.heartbeat) return false
...
function agentSummaryFromValue(value: unknown) {
  ...
  for (const key of ['summary', 'analysis', 'assessment', 'findings', 'corrections']) {
...
function SessionReferenceCard({ reference, onOpen }: { reference: ReferenceMedia; onOpen: (reference: ReferenceMedia) => void }) {
  ...
  return <article className="ps-session-reference">
    <button type="button" className={`ps-session-reference-preview ${reference.kind}`} onClick={() => onOpen(reference)} aria-label={`Open ${reference.name} in full view`}>
      {reference.kind === 'image' && <img src={reference.url} alt={reference.name} loading="lazy" />}
      {reference.kind === 'video' && <video src={reference.url} muted preload="metadata" />}
...
function ShotPlanModal({ shots, references, active, onClose, onEdit, onRetry, onOpenReference }: {
  shots: Shot[]; references: ReferenceMedia[]; active: boolean; onClose: () => void; onEdit: (shot: Shot) => void;
...
export function ProductionStudio({ csrf }: { csrf: string }) {
  ...
  const loadCatalog = async (refreshModels = false) => {
    ...
    const jobsTask = (async () => {
      try {
        const data = await jsonResponse(await fetch('/api/jobs', { cache: 'no-store' }))
  ...
  const liveTraces = cleanMessages.filter(message => message.kind === 'agent_trace')
  const latestTrace = liveTraces.length ? liveTraces[liveTraces.length - 1] : null
  const agentActivity = cleanMessages.filter(message => message.kind === 'agent_trace' || message.kind === 'agent')
  const latestActivity = agentActivity.length ? agentActivity[agentActivity.length - 1] : null
  const latestActivityDecision = typeof latestActivity?.metadata?.decision === 'string' ? latestActivity.metadata.decision : ''
  const latestActivityNext = typeof latestActivity?.metadata?.next_action === 'string' ? latestActivity.metadata.next_action : ''
  const latestTraceContent = latestTrace?.content.replace(/\s+/g, ' ').trim() || ''
  const latestTraceText = latestTrace
    ? `${latestTrace.participant.toUpperCase()}: ${latestTraceContent.length > 220 ? `${latestTraceContent.slice(0, 220)}…` : latestTraceContent}`
    : ''
  const latestActivityText = latestActivity?.kind === 'agent'
    ? `${latestActivity.participant.toUpperCase()}: ${compactAgentText(latestActivityDecision || latestActivityNext || latestActivity.content)}`
    : latestTraceText
  const generationProgressText = generationProgress
    ...
    : ''
  const latestHeartbeat = [...cleanMessages].reverse().find(message => message.kind === 'agent_trace' && message.metadata?.heartbeat === true)
  const conversationMessages = cleanMessages.filter(message => message.kind !== 'agent_trace' || (showLivePrints && (message.metadata?.stream === 'response' || (!message.metadata?.heartbeat || message.sequence === latestHeartbeat?.sequence))))
  ...
  return <section className="ps-root">
    ...
    {showProductionLoading ? <section className="ps-panel ps-loading-panel" role="status" aria-live="polite"><LoaderCircle className="spin" size={25} /><div><h3>Loading production</h3><p>The production room is loading independently. Other app sections remain available while the selected production data arrives.</p></div></section> : showProductionError ? <section className="ps-panel ps-loading-panel ps-loading-error" role="alert"><X size={25} /><div><h3>Production could not be loaded</h3><p>{productionLoadError}</p><button className="ps-secondary" onClick={() => void loadProduction(selectedId)}>Retry production</button></div></section> : (newProject || !production) ? <section className="ps-create ps-panel">
      ...
      {showShotPlan && production.shots?.length ? <ShotPlanModal shots={production.shots} references={production.references || []} active={!!active} onClose={() => setShowShotPlan(false)} onEdit={openShotEditor} onRetry={shot => void retryShot(shot)} onOpenReference={setPreviewReference} /> : null}
