...
function AgentSelect({ label, catalogs, runtime, model, effort, onRuntime, onModel, onEffort, mediaSeat = false }: {
  label: string; catalogs: Record<Runtime, ModelOption[]>; runtime: Runtime; model: string; effort: string;
  onRuntime: (value: Runtime) => void; onModel: (value: string) => void; onEffort: (value: string) => void; mediaSeat?: boolean
}) {
  ...
  return <div className="ps-agent-select">
    ...
    <label><span>Runtime</span><select value={runtime} disabled={mediaSeat} onChange={event => { const value = event.target.value as Runtime; const first = catalogs[value]?.[0]; onRuntime(value); if (first) { onModel(first.id); onEffort(first.efforts[0] || 'medium') } }}>
...
export function ProductionStudio({ csrf }: { csrf: string }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      form.set('codex_runtime', codexRuntime)
  ...
  const saveProductionConfig = async () => {
    ...
    try {
      const updated = await mutate(`/api/productions/${production.id}/settings`, 'PATCH', JSON.stringify({
        ...
        codex_runtime: configDraft.codex_runtime, codex_model: configDraft.codex_model,
    ...
    <div className="ps-settings-grid">
      <section className="ps-panel"><div className="ps-panel-title"><Bot size={18} /><div><h3>Agent defaults</h3><p>Used by new productions. Active projects keep their frozen configuration.</p></div></div>
        <div className="ps-agent-grid">
          <AgentSelect label="PRODUCER A" catalogs={models} runtime={settings.codex_runtime} model={settings.codex_model} effort={settings.codex_effort} onRuntime={value => setSettings(current => ({ ...current, codex_runtime: value }))} onModel={value => setSettings(current => ({ ...current, codex_model: value }))} onEffort={value => setSettings(current => ({ ...current, codex_effort: value }))} />
          <AgentSelect label="MEDIA PRODUCER" catalogs={models} runtime="agy" model={settings.agy_model} effort={settings.agy_effort} mediaSeat onRuntime={() => {}} onModel={value => setSettings(current => ({ ...current, agy_runtime: 'agy', agy_model: value }))} onEffort={value => setSettings(current => ({ ...current, agy_effort: value }))} />
  ...
  return <section className="ps-root">
    ...
    {(newProject || !production) ? <section className="ps-create ps-panel">
      ...
      <div className="ps-agent-grid"><AgentSelect label="PRODUCER A" catalogs={models} runtime={codexRuntime} model={codexModel} effort={codexEffort} onRuntime={setCodexRuntime} onModel={setCodexModel} onEffort={setCodexEffort} /><AgentSelect label="MEDIA PRODUCER" catalogs={models} runtime="agy" model={agyModel} effort={agyEffort} mediaSeat onRuntime={() => {}} onModel={setAgyModel} onEffort={setAgyEffort} /></div>
